...
$(document).ready(function () {
    ...
    for (let i = 1; i <= 100; i++) {
        ...
        let row = $(`<div class="row-name" class="rowId-${i}">${i}</div>`)
...
function updateCell(property, value, defaultPossible) {
    $(".input-cell.selected").each(function () {
        ...
        if (defaultPossible && JSON.stringify(cellData[selectedSheet][rowId][colId]) === JSON.stringify(defaultProperties)) {
            ...
            if (Object.keys(cellData[selectedSheet][rowId]) == 0) {
...
function addSheetEvents() {
    ...
    $(".cancel-button").click(function(){$(".sheet-rename-modal").remove();});
    ...
    //    $(".sheet-tab.selected").prev().remove();
       }
       else
       {
       $(".sheet-tab.selected").prev().click();
    //    $(".sheet-tab.selected").next().remove();
       }
    ...
    //    console.log(selectedSheet);
     ...
     {
        alert("Sorry, there is only 1 sheet! So it is not possible");
     }
    });
}
